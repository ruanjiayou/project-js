...
describe('test所有预期接口', () => {
  ...
  it('get /test/excel', async () => {
    const { body: pages } = await app.curl.get('/test/excel');
    assert.deepStrictEqual(pages, [
      { name: 'test1', data: [{ c1: 123, c2: 'abc' }, { c1: 456, c2: 'edf' }] }
    ]);
  });
  it('post /test/excel', async () => {
    const { text } = await app.curl.post('/test/excel').send([{ name: 'test', header: ['c1', 'c2'], data: [{ c1: 123, c2: 'abc' }, { c1: 456, c2: 'edf' }] }]);
    assert(typeof text === 'string');
  });
  it('post /test/base64-encode', async () => {
    const { text } = await app.curl.post('/test/base64-encode').send({ string: 'test' });
    // test => dGVzdA==
    assert.strictEqual(text, 'dGVzdA==');
  });
  it('post /test/base64-decode', async () => {
    const { text } = await app.curl.post('/test/base64-decode').send({ string: 'dGVzdA==' });
    // dGVzdA== => test
