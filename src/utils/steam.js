...
class GetBuffer extends Writable {
  constructor(options) {
    super(options);
    this.buff = Buffer.from([]);
  }
  _write(chunk) {
    this.buff = Buffer.concat([this.buff, chunk], this.buff.length + chunk.length);
  }
}

async function FileBuffer(fr) {
  const buff = await new Promise(function (resolve, reject) {
    const fb = new GetBuffer();
    // file stream
